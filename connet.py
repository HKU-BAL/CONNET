...
PHASE1_BATCHSIZE = 100000
PHASE2_BATCHSIZE = 100000
...
def main():
    ...
    for R in range(0, N_ITER):
        ...
        exe("python2 %s/split_job.py %d %d" % (DIR,R, PHASE1_BATCHSIZE))
        exe("parallel -j%d taskset -c {%%} python2 %s/%s/phase1.py %s {} :::: %d.parallel " % (T, DIR, "", MODEL1, R))
        exe("python2 %s/merge.py %d" % (DIR,R))
        ...
        exe("python2 %s/split_job.py %d.phase1 %d" % (DIR,R, PHASE2_BATCHSIZE))
        exe("parallel -j%d taskset -c {%%} python2 %s/%s/phase2.py %s {} :::: %d.phase1.parallel " % (T, DIR, "", MODEL2, R))
        exe("parallel python2 %s/apply_ins.py {.} %d ::: %d.phase1.*.ins.list.aaa" % (DIR,PHASE2_BATCHSIZE, R))
